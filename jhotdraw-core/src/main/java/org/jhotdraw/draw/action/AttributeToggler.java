...
public class AttributeToggler<T> extends AbstractAction {
  ...
  public void actionPerformed(ActionEvent evt) {
    if (compatibleTextAction != null) {
      handleCompatibleTextAction(evt);
      return;
    }

    T toggleValue = determineToggleValue();
    ArrayList<Figure> selectedFigures = new ArrayList<>(getView().getSelectedFigures());
    ArrayList<Object> restoreData = new ArrayList<>(selectedFigures.size());

    for (Figure figure : selectedFigures) {
      restoreData.add(figure.attr().getAttributesRestoreData());
      updateFigureAttribute(figure, toggleValue);
    }

    UndoableEdit edit = createUndoableEdit(selectedFigures, restoreData, toggleValue);
    getView().getDrawing().fireUndoableEditHappened(edit);
  }

  private void handleCompatibleTextAction(ActionEvent evt) {
  ...
  private T determineToggleValue() {
  ...
  private void updateFigureAttribute(Figure figure, T newValue) {
    figure.willChange();
    figure.attr().set(key, newValue);
    figure.changed();
  }

  private UndoableEdit createUndoableEdit(final ArrayList<Figure> selectedFigures,
      final ArrayList<Object> restoreData, final T newValue) {
