...
public abstract class AbstractDrawingView implements DrawingView, EditableComponent {
  ...
  private void validateHandles() {
    ...
    if (!handlesAreValid && getEditor() != null) {
      ...
      selectionHandles.clear();
      Rectangle invalidatedArea = null;
      detailLevel = calculateHandleDetailLevel();
      if (detailLevel != -1) {
        invalidatedArea = updateHandles();
      }
      if (invalidatedArea != null) {
        repaint(invalidatedArea);
      }
  ...
  private int calculateHandleDetailLevel() {
    for (int level = detailLevel; level >= 0; level--) {
      if (hasHandlesAtDetailLevel(level)) {
        return level;
      }
    }
    return -1; // No handles available at any detail level
  }

  private boolean hasHandlesAtDetailLevel(int level) {
    for (Figure figure : getSelectedFigures()) {
      if (figure.createHandles(level).size() > 0) {
        return true;
      }
    }
    return false;
  ...
  private Rectangle updateHandles() {
    Rectangle invalidatedArea = null;
    for (Figure figure : getSelectedFigures()) {
