...
public class DefaultDrawingViewTransferHandler extends TransferHandler {
  ...
  protected Transferable createTransferable(
      DrawingView view, java.util.Set<Figure> transferFigures) {
        ...
        if (drawing.getOutputFormats() == null || drawing.getOutputFormats().size() == 0) {
          ...
          if (toBeCopied.size() > 0) {
            try {
              return createCompositeTransferable(drawing, toBeCopied, view.getScaleFactor());
            } catch (IOException e) {
              e.printStackTrace();
              return null;
            }
      ...
      private Transferable createCompositeTransferable(Drawing drawing, List<Figure> toBeCopied, double scaleFactor) throws IOException {
        CompositeTransferable transfer = new CompositeTransferable();
        for (OutputFormat format : drawing.getOutputFormats()) {
          Transferable t = format.createTransferable(drawing, toBeCopied, scaleFactor);
          if (!transfer.isDataFlavorSupported(t.getTransferDataFlavors()[0])) {
            transfer.add(t);
          }
        }
        exportedFigures = new HashSet<>(toBeCopied);
        return transfer;
      }
//Refactoring end
  }
