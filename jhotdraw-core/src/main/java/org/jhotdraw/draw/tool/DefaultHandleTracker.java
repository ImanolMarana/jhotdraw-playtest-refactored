...
public class DefaultHandleTracker extends AbstractTool implements HandleTracker {
  ...
  protected void updateHoverHandles(DrawingView view, Figure f) {
        ...
        if (hoverFigure != null && hoverFigure.isSelectable()) {
            r = clearHoverHandlesAndCalculateBounds();
        }
        ...
        if (hoverFigure != null) {
            ...
            r = setHandlesViewAndCalculateBounds(view, r);
        }
    ...
    private Rectangle clearHoverHandlesAndCalculateBounds() {
        Rectangle r = null;
        for (Handle h : hoverHandles) {
            if (r == null) {
                r = h.getDrawingArea();
    ...
    private Rectangle setHandlesViewAndCalculateBounds(DrawingView view, Rectangle r) {
        for (Handle h : hoverHandles) {
            h.setView(view);
            if (r == null) {
                r = h.getDrawingArea();
