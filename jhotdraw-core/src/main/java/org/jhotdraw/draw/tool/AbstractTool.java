  ...
  */
  ...
  public void keyPressed(KeyEvent evt) {
        if (!evt.isConsumed()) {
            ...
            ActionListener al = getActionListener(evt);
            if (al != null) {
                evt.consume();
    ...
    private ActionListener getActionListener(KeyEvent evt) {
        Object obj = getActionKeyFromInputMaps(evt);
        if (obj instanceof ActionListener) {
            return (ActionListener) obj;
        } else if (obj != null) {
            return getActionListenerFromActionMaps(obj);
        }
        return null;
    }
    
    private Object getActionKeyFromInputMaps(KeyEvent evt) {
        if (inputMap != null) {
            // Lookup the input map of the tool
            return inputMap.get(KeyStroke.getKeyStroke(evt.getKeyCode(), evt.getModifiers(), false));
        } else {
            // Fall back to the input map of the drawing editor
            InputMap im = editor.getInputMap();
            if (im != null) {
                return im.get(KeyStroke.getKeyStroke(evt.getKeyCode(), evt.getModifiers(), false));
            }
        }
        return null;
    }

    private ActionListener getActionListenerFromActionMaps(Object obj) {
        // Lookup the action map of the tool
        if (actionMap != null) {
            ActionListener al = actionMap.get(obj);
            if(al != null) {
                return al;
            }
        }
        // Fall back to the action map of the drawing editor
        return editor.getActionMap().get(obj);        
    }
