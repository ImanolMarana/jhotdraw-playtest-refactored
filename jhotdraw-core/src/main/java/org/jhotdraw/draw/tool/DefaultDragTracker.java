 ...
 * Name: State.<br>
 * Role: State.<br>
 * Partners: {@link SelectAreaTracker} as State, {@link SelectionTool} as Context, {@link
...
public class DefaultDragTracker extends AbstractTool implements DragTracker {
  ...
  public void mousePressed(MouseEvent evt) {
    ...
    handleShiftClick(view);

    if (!view.getSelectedFigures().isEmpty()) {
      initializeDragging(view);
    }
  }

  private void handleShiftClick(DrawingView view) {
    if (editor.getInputFormat().isShiftDown()) {
  ...
  private void initializeDragging(DrawingView view) {
    dragRect = null;
