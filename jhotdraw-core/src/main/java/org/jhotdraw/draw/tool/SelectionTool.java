...
public class SelectionTool extends AbstractTool {
  ...
  private void handleMousePressed(MouseEvent evt) {
    ...
    if (handle != null) {
      ...
      newTracker = getTrackerForFigure(evt);
    }
  ...
  private Tool getTrackerForFigure(MouseEvent evt) {
    DrawingView view = getView();
    Figure figure = findFigure(evt);
  ...
  private Figure findFigure(MouseEvent evt) {
    DrawingView view = getView();
    ...
    if (isSelectBehindEnabled()
        ...
        != 0) {
      return findFigureBehind(view, p);
    } else {
      return findFigureNormally(view, drawing, p);
    }
  }

  private Figure findFigureBehind(DrawingView view, Point2D.Double p) {
    Drawing drawing = view.getDrawing();
    Figure figure = drawing.findFigure(p, view.getScaleFactor());
  ...
  private Figure findFigureNormally(DrawingView view, Drawing drawing, Point2D.Double p) {
    Figure figure = null;
    if (isSelectBehindEnabled()) {
