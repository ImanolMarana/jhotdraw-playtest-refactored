...
public class DelegationSelectionTool extends SelectionTool {
  ...
  protected void handleDoubleClick(MouseEvent evt) {
        ...
        handleDoubleClickOnFigure(evt, v, pos);
    }
    
    private void handleDoubleClickOnFigure(MouseEvent evt, DrawingView v, Point pos) {
        Point2D.Double p = viewToDrawing(pos);
        Figure figure = findFigureForDoubleClick(v, p);
        if (figure != null && figure.isSelectable()) {
            Tool figureTool = figure.getTool(p);
            if (figureTool == null && !figure.contains(p)) {
                figure = getDrawing().findFigureInside(p);
                if (figure != null) {
                    figureTool = figure.getTool(p);
                }
            }
            handleDoubleClickWithTool(v, evt, figure, figureTool);
        }
        evt.consume();
    }
    
    private Figure findFigureForDoubleClick(DrawingView v, Point2D.Double p) {
        Figure figure = null;
        ...
        if (figure == null) {
            figure = v.findFigure(viewToDrawing(p));
        }
        return figure;
    }
    
    private void handleDoubleClickWithTool(DrawingView v, MouseEvent evt, Figure outerFigure, Tool figureTool) {
        if (figureTool != null) {
            setTracker(figureTool);
            figureTool.mousePressed(evt);
        } else {
            if (!outerFigure.handleMouseClick(viewToDrawing(evt.getPoint()), evt, getView())) {
                v.clearSelection();
                v.addToSelection(outerFigure);
                v.setHandleDetailLevel(v.getHandleDetailLevel() + 1);
            } else if (!v.getSelectedFigures().contains(outerFigure)){
                v.clearSelection();
                v.addToSelection(outerFigure);
            }
        }
