...
public class DefaultSelectAreaTracker extends AbstractTool implements SelectAreaTracker {
  ...
  protected void updateHoverHandles(DrawingView view, Figure f) {
        ...
        Rectangle r = updateHoverHandlesDisposePrevious(view);

        hoverFigure = f;
        updateHoverHandlesCreateNew(view, f, r);
    }
    
    private Rectangle updateHoverHandlesDisposePrevious(DrawingView view) {
        Rectangle r = null;
        if (hoverFigure != null) {
            for (Handle h : hoverHandles) {
                if (r == null) {
    ...
    private void updateHoverHandlesCreateNew(DrawingView view, Figure f, Rectangle r) {
