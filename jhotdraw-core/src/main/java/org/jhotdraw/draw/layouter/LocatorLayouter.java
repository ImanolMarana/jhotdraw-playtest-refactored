...
public class LocatorLayouter implements Layouter {
  ...
  public Rectangle2D.Double layout(
        ...
        Rectangle2D.Double bounds = null;
        for (Figure child : extractFiguresToLayout(compositeFigure)) {
            bounds = layoutChild(child, compositeFigure, bounds, scale);
        }
        return (bounds == null) ? new Rectangle2D.Double() : bounds;
    }
    
    private Rectangle2D.Double layoutChild(Figure child, CompositeFigure compositeFigure, Rectangle2D.Double bounds, double scale) {
        Locator locator = getLocator(child);
        Rectangle2D.Double r;
        ...
        if (position != null) {
            applyRotation(child, position);
        }

        child.changed();
        return updateBounds(bounds, r);
    }

    private Rectangle2D.Double updateBounds(Rectangle2D.Double bounds, Rectangle2D.Double r) {
        if (!r.isEmpty()) {
            if (bounds == null) {
                bounds = r;
            } else {
    ...
    private void applyRotation(Figure child, Position position) {
