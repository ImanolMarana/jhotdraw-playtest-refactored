...
public abstract class AbstractConnectionHandle extends AbstractHandle {
  ...
  public final Collection<Handle> createSecondaryHandles() {
          ...
          if (getOwner().getLiner() == null && savedLiner == null) {
              list.addAll(createBezierControlPointHandles());
          }
          return list;
      }
    
      private Collection<Handle> createBezierControlPointHandles() {
          Collection<Handle> list = new ArrayList<>();
          int index = getBezierNodeIndex();
          ...
          list.addAll(createNeighborControlPointHandles(f, index));
    
          return list;
      }
    
      private Collection<Handle> createNeighborControlPointHandles(BezierFigure f, int index) {
          Collection<Handle> list = new ArrayList<>();
