...
public class BezierNodeHandle extends AbstractHandle implements CoordinateDataSupplier {
  ...
  public Collection<Handle> createSecondaryHandles() {
    ...
    addControlPointHandles(f, list, v);

    addAdjacentControlPointHandles(f, list);

    return list;
  }

  private void addControlPointHandles(BezierFigure f, Collection<Handle> list, BezierPath.Node v) {
  ...
  private void addAdjacentControlPointHandles(BezierFigure f, Collection<Handle> list) {
    if (index > 0 || f.isClosed()) {
      int i = (index == 0) ? f.getNodeCount() - 1 : index - 1;
      BezierPath.Node prevNode = f.getNode(i);
      if ((prevNode.mask & BezierPath.C2_MASK) != 0) {
    ...
    if (index < f.getNodeCount() - 1 || f.isClosed()) {
      ...
      BezierPath.Node nextNode = f.getNode(i);
      if ((nextNode.mask & BezierPath.C1_MASK) != 0) {
