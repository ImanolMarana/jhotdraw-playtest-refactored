...
public class BezierControlPointHandle extends AbstractHandle {
  ...
  public void draw(Graphics2D g) {
    ...
    if (f.getNodeCount() > index) {
      ...
      drawTangentLines(g, p0, pc);
      drawControlPointCircle(g, v, p0, pc);
    }
  }

  private void drawTangentLines(Graphics2D g, Point2D.Double p0, Point2D.Double pc) {
    Color strokeColor1 = getTangentColor(HandleAttributeKeys.BEZIER_TANGENT_COLOR_1,
        HandleAttributeKeys.BEZIER_TANGENT_COLOR_1_DISABLED);
    Stroke stroke1 = getTangentStroke(HandleAttributeKeys.BEZIER_TANGENT_STROKE_1,
        HandleAttributeKeys.BEZIER_TANGENT_STROKE_1_DISABLED);

    Color strokeColor2 = getTangentColor(HandleAttributeKeys.BEZIER_TANGENT_COLOR_2,
        HandleAttributeKeys.BEZIER_TANGENT_COLOR_2_DISABLED);
    Stroke stroke2 = getTangentStroke(HandleAttributeKeys.BEZIER_TANGENT_STROKE_2,
        HandleAttributeKeys.BEZIER_TANGENT_STROKE_2_DISABLED);
  ...
  private void drawControlPointCircle(Graphics2D g, BezierPath.Node v, Point2D.Double p0,
      Point2D.Double pc) {
    Color handleFillColor = getHandleFillColor();
    Color handleStrokeColor = getHandleStrokeColor();

    if (v.keepColinear
        && v.mask == BezierPath.C1C2_MASK
        && (index > 0 && index < getBezierFigure().getNodeCount() - 1
            || getBezierFigure().isClosed())) {
  ...
  private Color getTangentColor(Object enabledKey, Object disabledKey) {
    return getEditor().getTool().supportsHandleInteraction()
        ? getEditor().getHandleAttribute(enabledKey)
        : getEditor().getHandleAttribute(disabledKey);
  }

  private Stroke getTangentStroke(Object enabledKey, Object disabledKey) {
    return getEditor().getTool().supportsHandleInteraction()
        ? getEditor().getHandleAttribute(enabledKey)
        : getEditor().getHandleAttribute(disabledKey);
  }

  private Color getHandleFillColor() {
    return getEditor().getTool().supportsHandleInteraction()
        ? getEditor()
  ...
  private Color getHandleStrokeColor() {
    return getEditor().getTool().supportsHandleInteraction()
        ? getEditor()
