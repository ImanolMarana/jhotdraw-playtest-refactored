...
public class LinearLabelLocator implements Locator {
  ...
  protected Locator.Position getRelativeLabelPoint(
        ...
        return calculateAdjustedPosition(point, p, labelDim, position.angle());
      }
    
      private Locator.Position calculateAdjustedPosition(
          Point2D.Double point,
          Point2D.Double p,
          Dimension2DDouble labelDim,
          double angle) {
        if (relativePosition == 0.5
            && p.x >= point.x - distance / 2
            && p.x <= point.x + distance / 2) {
          return p.y >= point.y
              ? new Locator.Position(
                  new Point2D.Double(p.x - labelDim.width / 2, p.y), angle)
              : new Locator.Position(
                  new Point2D.Double(p.x - labelDim.width / 2, p.y - labelDim.height), angle);
        } else {
          if (p.x >= point.x) {
            return p.y >= point.y
                ? new Locator.Position(new Point2D.Double(p.x, p.y), angle)
                : new Locator.Position(
                    new Point2D.Double(p.x, p.y - labelDim.height), angle);
          } else {
            return p.y >= point.y
                ? new Locator.Position(
                    new Point2D.Double(p.x - labelDim.width, p.y), angle)
                : new Locator.Position(
                    new Point2D.Double(p.x - labelDim.width, p.y - labelDim.height),
                    angle);
          }
        }
      }
