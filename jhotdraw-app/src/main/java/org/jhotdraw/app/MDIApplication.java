...
public class MDIApplication extends AbstractApplication {
  ...
  protected ActionMap createModelActionMap(ApplicationModel mo) {
    ...
    rootMap.put(
        ArrangeWindowsAction.VERTICAL_ID,
        new ArrangeWindowsAction(desktopPane, Arrangeable.Arrangement.VERTICAL));
    rootMap.put(
        ArrangeWindowsAction.HORIZONTAL_ID,
        new ArrangeWindowsAction(desktopPane, Arrangeable.Arrangement.HORIZONTAL));
    rootMap.put(
        ArrangeWindowsAction.CASCADE_ID,
        new ArrangeWindowsAction(desktopPane, Arrangeable.Arrangement.CASCADE));
  ...
  protected JMenuBar createMenuBar(View v) {
        ...
        JMenu fileMenu = getMenuFromModel(mb, v);
        JMenu editMenu = getMenuFromModel(mb, v);
        JMenu viewMenu = getMenuFromModel(mb, v);
        JMenu windowMenu = getMenuFromModel(mb, v);
        JMenu helpMenu = getMenuFromModel(mb, v);
        ...
        insertMenuIfNotNull(mb, fileMenu, 0);
        insertMenuIfNotNull(mb, editMenu, Math.min(1, mb.getComponentCount()));
        insertMenuIfNotNull(mb, viewMenu, Math.min(2, mb.getComponentCount()));
        insertMenuIfNotNull(mb, windowMenu, mb.getComponentCount());
        insertMenuIfNotNull(mb, helpMenu, mb.getComponentCount());

        return mb;
    }

    private JMenu getMenuFromModel(JMenuBar mb, View v) {
    ...
    private void insertMenuIfNotNull(JMenuBar mb, JMenu menu, int position) {
        if (menu != null) {
            mb.add(menu, position);
        }
    }
//Refactoring end
  ...
  private class WindowMenuHandler implements PropertyChangeListener {
    ...
    public void propertyChange(PropertyChangeEvent evt) {
      String name = evt.getPropertyName();
      if (((name == null && VIEW_COUNT_PROPERTY == null)
              || (name != null && name.equals(VIEW_COUNT_PROPERTY)))
          || "paletteCount".equals(name)) {
        updateWindowMenu();
      }
    }
    ...
    public boolean importData(JComponent comp, Transferable t) {
      ...
      try {
        ...
        for (final File f : files) {
          a.actionPerformed(
              new ActionEvent(desktopPane, ActionEvent.ACTION_PERFORMED, f.toString()));
        }
        return true;
