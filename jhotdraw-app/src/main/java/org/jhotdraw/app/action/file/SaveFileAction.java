 ...
 * ApplicationModel} in method {@link ApplicationModel#initApplication}. <hr> <b>Features</b>
 *
...
public class SaveFileAction extends AbstractViewAction {
  ...
  private void handleSaveApproved(SheetEvent evt, View view) {
    final URI uri = evt.getChooser().getSelectedURI();
    if (!isUriInUse(uri, view)) {
      saveViewToURI(view, uri, evt.getChooser());
    } else {
      showUriInUseMessage(evt, view);
    }
  }

  private boolean isUriInUse(URI uri, View currentView) {
    if (getApplication().getModel().isAllowMultipleViewsPerURI()) {
      return false;
    }
    for (View v : getApplication().getViews()) {
      if (v != currentView && v.getURI() != null && v.getURI().equals(uri)) {
        return true;
      }
    }
    return false;
  ...
  private void showUriInUseMessage(SheetEvent evt, View view) {
      ...
      protected void failed(Throwable value) {
        ...
        JSheet.showMessageSheet(
            ...
            "<html>"
                ...
                + "<b>"
                + labels.getFormatted("file.save.couldntSave.message", URIUtil.getName(file))
                + "</b><p>"
