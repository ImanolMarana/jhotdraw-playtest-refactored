 ...
 *
 * <p>This action is called when the user drops a file on the dock icon of {@code
 * DefaultOSXApplication} or onto the desktop area of {@code DefaultMDIApplication}.
 *
 ...
 * org.jhotdraw.app.ApplicationModel#initApplication}. <hr> <b>Features</b>
 *
 * <p>
 *
...
public class OpenApplicationFileAction extends AbstractApplicationAction {
  ...
  protected void openView(final View view, final URI uri) {
        final Application app = getApplication();
        app.setEnabled(true);
        view.setMultipleOpenId(calculateMultipleOpenId(view, uri, app));
        view.setEnabled(false);
        new SwingWorker<Object, Void>() {
            @Override
            protected Object doInBackground() throws Exception {
                return loadFile(view, uri);
            }

            @Override
            protected void done() {
                handleFileLoadResult(view, uri, app);
            }
        }.execute();
    }
    
    private int calculateMultipleOpenId(View view, URI uri, Application app) {
    ...
    private Object loadFile(View view, URI uri) throws Exception {
        boolean exists = true;
        try {
            File f = new File(uri);
            exists = f.exists();
    ...
    private void handleFileLoadResult(View view, URI uri, Application app) {
        try {
            get();
            view.setURI(uri);
