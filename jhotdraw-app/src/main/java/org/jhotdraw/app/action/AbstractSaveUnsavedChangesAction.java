 ...
 * cancel or save the changes before doing it. If the user chooses to discard the changes, {@code
...
public abstract class AbstractSaveUnsavedChangesAction extends AbstractViewAction {
  ...
  public void actionPerformed(ActionEvent evt) {
        ...
        final View v = av;
        if (v.isEnabled()) {
            handleSaveUnsavedChanges(v);
        }
    }
    
    private void handleSaveUnsavedChanges(final View v) {
        ...
        if (v.hasUnsavedChanges()) {
            showSaveConfirmationDialog(v, labels);
        } else {
            proceedWithAction(v);
        }
    }
    
    private void showSaveConfirmationDialog(final View v, final ResourceBundleUtil labels) {
        ...
        JSheet.showSheet(pane, v.getComponent(), new SheetListener() {
            ...
            public void optionSelected(SheetEvent evt) {
                handleSaveConfirmation(evt, v, labels);
            }
        });
    }
    
    private void handleSaveConfirmation(SheetEvent evt, View v, ResourceBundleUtil labels) {
    ...
    private void proceedWithAction(View v) {
        doIt(v);
        v.setEnabled(true);
    ...
    new SwingWorker() {
      ...
      protected void done() {
        try {
          ...
          Logger.getLogger(AbstractSaveUnsavedChangesAction.class.getName())
