...
private boolean handleUnsavedViews() {
    final Application app = getApplication();
    ...
    switch (unsavedViewsCount) {
        case 1:
            handleSingleUnsavedView(documentToBeReviewed, unsavedURI, labels);
            break;
        case 0:
            break;
        default:
            handleMultipleUnsavedViews(labels);
            break;
    }
    return false;
}

private void handleSingleUnsavedView(View documentToBeReviewed, URI unsavedURI, ResourceBundleUtil labels) {
    final Application app = getApplication();
    ...
    JOptionPane pane = createUnsavedViewOptionPane(unsavedURI, labels);
    showUnsavedViewOptionPane(pane, labels);
}

private void handleMultipleUnsavedViews(ResourceBundleUtil labels) {
    final Application app = getApplication();
    JOptionPane pane = createMultipleUnsavedViewsOptionPane(labels);
    JDialog dialog = pane.createDialog(app.getComponent(), null);
    positionDialog(dialog);
    dialog.setVisible(true);
    handleMultipleUnsavedViewsResult(pane.getValue(), labels);
}

private JOptionPane createUnsavedViewOptionPane(URI unsavedURI, ResourceBundleUtil labels) {
...
private void showUnsavedViewOptionPane(JOptionPane pane, final ResourceBundleUtil labels) {
    final Application app = getApplication();
    Object[] options = {
...
private JOptionPane createMultipleUnsavedViewsOptionPane(ResourceBundleUtil labels) {
    final Application app = getApplication();
    int unsavedViewsCount = getUnsavedViewsCount();
    return new JOptionPane(
            "<html>"
...
private void positionDialog(JDialog dialog) {
    final Application app = getApplication();
    Rectangle screenBounds = dialog.getGraphicsConfiguration().getBounds();
    if (app.getComponent() == null
            || !screenBounds.contains(app.getComponent().getBounds())) {
        // place dialog in center and in upper third of screen
        dialog.setLocation(
                (screenBounds.width - dialog.getWidth()) / 2,
                (screenBounds.height - dialog.getHeight()) / 3);
    }
}

private void handleMultipleUnsavedViewsResult(Object value, ResourceBundleUtil labels) {
    final Application app = getApplication();
    if (value == null || value.equals(labels.getString("application.exit.cancelOption"))) {
        ...
        unsavedView = getFirstUnsavedView();
        reviewChanges();
    }
}

private int getUnsavedViewsCount() {
    int unsavedViewsCount = 0;
    ...
    return unsavedViewsCount;
}

private View getFirstUnsavedView() {
    for (View p : getApplication().views()) {
        if (p.hasUnsavedChanges() && p.isEnabled()) {
            return p;
        }
    }
    return null;
        ...
        try {
          ...
          JSheet.showMessageSheet(
              ...
              "<html>"
                  + UIManager.getString("OptionPane.css")
                  + "<b>"
                  + labels.format("file.save.couldntSave.message", URIUtil.getName(uri))
                  + "</b><p>"
                  + ex,
        ...
        try {
          ...
          JSheet.showMessageSheet(
              ...
              "<html>"
                  + UIManager.getString("OptionPane.css")
                  + "<b>"
                  + labels.format("file.save.couldntSave.message", uri)
                  + "</b><p>"
                  + ex,
              JOptionPane.ERROR_MESSAGE);
          v.setEnabled(true);
