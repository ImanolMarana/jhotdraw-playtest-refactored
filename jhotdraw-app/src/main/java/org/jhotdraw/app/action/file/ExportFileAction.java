...
public class ExportFileAction extends AbstractViewAction {
  ...
  private void prepareExport(final View view, ResourceBundleUtil labels) {
    URIChooser fileChooser = getApplication().getExportChooser(view);
    if (proposeFileName) {
      proposeFileName(view, fileChooser);
    }
    showExportChooser(view, labels, fileChooser);
  }
  
  private void proposeFileName(View view, URIChooser fileChooser) {
    // => try to propose file name without extension
    URI proposedURI = view.getURI();
    if (proposedURI != null) {
      try {
        URI selectedURI = fileChooser.getSelectedURI();
        File selectedFolder = getSelectedFolder(proposedURI, selectedURI);
  ...
  private void showExportChooser(final View view, ResourceBundleUtil labels, URIChooser fileChooser) {
    JSheet.showSheet(
        fileChooser,
        view.getComponent(),
        labels.getString("filechooser.export"),
        new SheetListener() {
          @Override
          public void optionSelected(final SheetEvent evt) {
            handleExportOption(view, evt);
          }
        });
  }
  
  private void handleExportOption(final View view, final SheetEvent evt) {
  ...
  private File getSelectedFolder(URI proposedURI, URI selectedURI) throws URISyntaxException {
    if (selectedURI == null) {
      Preferences prefs =
          Preferences.userNodeForPackage(getApplication().getModel().getClass());
      try {
        ...
        return new File(selectedURI).getParentFile();
      } catch (URISyntaxException ex) {
        // selectedURI is null
        return new File(proposedURI).getParentFile();
      }
    } else {
      return new File(selectedURI).getParentFile();
    }
...
//Refactoring end
  ...
  protected void exportView(final View view, final URI uri, final URIChooser chooser) {
    new SwingWorker() {
      @Override
