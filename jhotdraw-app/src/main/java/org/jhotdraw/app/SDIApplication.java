 ...
 *
 * <p>The life cycle of the application is tied to the {@code JFrame}s. Closing the last {@code
 * JFrame} quits the application.
 *
...
public class SDIApplication extends AbstractApplication {
  ...
  protected JMenuBar createMenuBar(View v) {
        ...
        // Retrieve menus from application model
        JMenu fileMenu = getMenuFromModel(v, "file.text");
        JMenu editMenu = getMenuFromModel(v, "edit.text");
        JMenu viewMenu = getMenuFromModel(v, "view.text");
        JMenu windowMenu = getMenuFromModel(v, "window.text");
        JMenu helpMenu = getMenuFromModel(v, "help.text");
        ...
        addMenuIfNotNull(mb, fileMenu, 0);
        addMenuIfNotNull(mb, editMenu, Math.min(1, mb.getComponentCount()));
        addMenuIfNotNull(mb, viewMenu, Math.min(2, mb.getComponentCount()));
        addMenuIfNotNull(mb, windowMenu);
        addMenuIfNotNull(mb, helpMenu);

        return mb;
    }

    private JMenu getMenuFromModel(View v, String menuTextKey) {
        JMenu foundMenu = null;
        String menuText = labels.getString(menuTextKey);
        for (JMenu mm : getModel().getMenuBuilder().addOtherMenus(new LinkedList<>(), this, v)) {
    ...
    private void addMenuIfNotNull(JMenuBar mb, JMenu menu) {
        if (menu != null) {
            mb.add(menu);
        }
    }

    private void addMenuIfNotNull(JMenuBar mb, JMenu menu, int index) {
        if (menu != null) {
            mb.add(menu, index);
        }
    }
