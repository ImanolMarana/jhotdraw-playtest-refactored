...
public class OSXApplication extends AbstractApplication {
  ...
  public void removeWindow(Window window) {
        ...
        unlinkMenuItemsFromActions(frame.getJMenuBar());
    ...
    private void unlinkMenuItemsFromActions(JMenuBar mb) {
        if (mb == null) {
            return;
        }
        
        Stack<JMenu> menuStack = new Stack<>();
        for (int i = 0, n = mb.getMenuCount(); i < n; ++i) {
            if (mb.getMenu(i) != null) {
                menuStack.push(mb.getMenu(i));
            }
        }

        while (!menuStack.isEmpty()) {
            JPopupMenu menu = menuStack.pop().getPopupMenu();
            for (int i = 0, n = menu.getComponentCount(); i < n; ++i) {
                Component comp = menu.getComponent(i);
                if (comp instanceof JMenu) {
                    menuStack.push((JMenu) comp);
  ...
  private class FrameHandler extends WindowAdapter implements PropertyChangeListener, Disposable {
    ...
    public void windowClosing(final WindowEvent evt) {
      getAction(view, CloseFileAction.ID)
          .actionPerformed(
              new ActionEvent(evt.getSource(), ActionEvent.ACTION_PERFORMED, "windowClosing"));
    }
