  ...
  private class Handler implements PropertyChangeListener {
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        if (evt.getSource() == source) {
            handleSourcePropertyChange(evt);
        } else if (bidirectional && evt.getSource() == target) {
            handleTargetPropertyChange(evt);
        }
    }

    private void handleSourcePropertyChange(PropertyChangeEvent evt) {
        if (sourceProperty.equals(evt.getPropertyName()) && target != null) {
            updateTargetValue(evt.getNewValue());
        }
    }

    private void handleTargetPropertyChange(PropertyChangeEvent evt) {
        if (targetProperty.equals(evt.getPropertyName()) && source != null) {
            updateSourceValue(evt.getNewValue());
        }
    }

    private void updateTargetValue(Object newValue) {
        try {
            getTargetWriteMethod().invoke(target, newValue);
        } catch (Exception ex) {
            throw new InternalError("Could not set property \"" + targetProperty + "\" on " + target, ex);
        }
    }

    private void updateSourceValue(Object newValue) {
        try {
            getSourceWriteMethod().invoke(source, newValue);
        } catch (Exception ex) {
            throw new InternalError("Could not set property \"" + sourceProperty + "\" on " + source, ex);
        }
    }
//Refactoring end
  ...
  private void addPropertyChangeListener(Object bean, PropertyChangeListener listener) {
    try {
      Method m =
          bean.getClass().getMethod("addPropertyChangeListener", PropertyChangeListener.class);
  ...
  private void removePropertyChangeListener(Object bean, PropertyChangeListener listener) {
    try {
      Method m =
          bean.getClass().getMethod("removePropertyChangeListener", PropertyChangeListener.class);
      m.invoke(bean, listener);
    } catch (Exception ex) {
      InternalError ie =
          new InternalError("Could not remove property change listener from " + bean);
      ie.initCause(ex);
      throw ie;
    }
  }
  ...
  private Method getTargetWriteMethod() {
    if (targetWriteMethod == null) {
      try {
        ...
        InternalError ie =
            new InternalError("Could not create target property descriptor for " + target);
        ie.initCause(ex);
        throw ie;
      }
    }
  ...
  private Method getSourceWriteMethod() {
    if (sourceWriteMethod == null) {
      try {
        ...
        InternalError ie =
            new InternalError("Could not create source property descriptor for " + source);
        ie.initCause(ex);
        throw ie;
      }
    }
