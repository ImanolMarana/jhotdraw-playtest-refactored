...
public class DoubleStroke implements Stroke {
  ...
  protected void traceStroke(BezierPath bp, Path2D.Double left, Path2D.Double right) {
        ...
        removeDuplicateNodes(bp);

        // Handle the first point of the bezier path
        handleFirstPoint(bp, left, right, currentCorners, prevCorners);

        // Handle points in the middle of the bezier path
        handleMiddlePoints(bp, left, right, currentCorners, prevCorners);

        // Handle the last point of the bezier path
        handleLastPoint(bp, left, right, currentCorners, prevCorners);
      }

      private void removeDuplicateNodes(BezierPath bp) {
        if (bp.isClosed()) {
          removeDuplicateNodesClosedPath(bp);
        } else {
          removeDuplicateNodesOpenPath(bp);
        }
      }

      private void removeDuplicateNodesClosedPath(BezierPath bp) {
      ...
      private void removeDuplicateNodesOpenPath(BezierPath bp) {
      ...
      private void handleFirstPoint(BezierPath bp, Path2D.Double left, Path2D.Double right,
          double[] currentCorners, double[] prevCorners) {
        if (bp.isClosed() && bp.size() > 1) {
          handleFirstPointClosedPath(bp, left, right, currentCorners, prevCorners);
        } else {
          handleFirstPointOpenPath(bp, left, right, currentCorners);
        }
      }

      private void handleFirstPointClosedPath(BezierPath bp, Path2D.Double left,
          Path2D.Double right, double[] currentCorners, double[] prevCorners) {
        ...
        currentCorners = computeThickLine(
            ...
            innerWidth,
            currentCorners);
        Point2D.Double intersect = Geom.intersect(
            prevCorners[0],
            prevCorners[1],
            prevCorners[4],
            prevCorners[5],
      ...
      private void handleFirstPointOpenPath(BezierPath bp, Path2D.Double left, Path2D.Double right,
      ...
      private void handleMiddlePoints(BezierPath bp, Path2D.Double left, Path2D.Double right,
          double[] currentCorners, double[] prevCorners) {
        for (int i = 1, n = bp.size() - 1; i < n; i++) {
          handleMiddlePoint(bp, left, right, currentCorners, prevCorners, i);
        }
      }

      private void handleMiddlePoint(BezierPath bp, Path2D.Double left, Path2D.Double right,
          double[] currentCorners, double[] prevCorners, int i) {
      ...
      private void handleLastPoint(BezierPath bp, Path2D.Double left, Path2D.Double right,
          double[] currentCorners, double[] prevCorners) {
        if (bp.isClosed() && bp.size() > 0) {
          handleLastPointClosedPath(bp, left, right, currentCorners, prevCorners);
        } else {
          handleLastPointOpenPath(bp, left, right, currentCorners);
        }
      }

      private void handleLastPointClosedPath(BezierPath bp, Path2D.Double left, Path2D.Double right,
          double[] currentCorners, double[] prevCorners) {
        double[] tmp = prevCorners;
        prevCorners = currentCorners;
        currentCorners = computeThickLine(
      ...
      private void handleLastPointOpenPath(BezierPath bp, Path2D.Double left, Path2D.Double right,
          double[] currentCorners) {
        if (bp.size() > 1) {
      ...
      private double[] computeThickLine(double[] seg, int offset, double corners[]) {
        return computeThickLine(
            seg[0 + offset], seg[1 + offset], seg[2 + offset], seg[3 + offset], innerWidth, corners);
      }

      private double[] computeThickLine(
          double x1, double y1, double x2, double y2, double thickness, double corners[]) {
        double dx = x2 - x1;
        double dy = y2 - y1;
        // line length
