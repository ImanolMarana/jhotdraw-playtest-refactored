...
public class Bezier {
   ...
   * @param error the maximal allowed error between the bezier path and the digitized points.
   */
   ...
   * @param error the maximal allowed error between the bezier path and the digitized points.
   */
  public static BezierPath fitBezierPath(
      java.util.List<Point2D.Double> digitizedPoints, double error) {
   ...
   * @param minDistance minimal distance between two points. If minDistance is 0, this method only
   *     removes sequences of coincident points.
   * @return Digitized points with a minimal distance.
   */
  public static ArrayList<Point2D.Double> removeClosePoints(
      java.util.List<Point2D.Double> digitizedPoints, double minDistance) {
    if (minDistance == 0) {
   ...
   */
  private static ArrayList<Point2D.Double> removeCoincidentPoints(
      java.util.List<Point2D.Double> digitizedPoints) {
    java.util.ArrayList<Point2D.Double> cleaned = new ArrayList<>();
    if (digitizedPoints.size() > 0) {
      Point2D.Double prev = digitizedPoints.get(0);
      cleaned.add(prev);
   ...
   *
   * @param digitizedPoints Digitized points
   * @param maxAngle maximal angle in radians between the current point and its predecessor and
   ...
   *     points.
   */
  public static ArrayList<ArrayList<Point2D.Double>> splitAtCorners(
      java.util.List<Point2D.Double> digitizedPoints, double maxAngle, double minDistance) {
   ...
   * @param minDistance Minimal distance between a point and adjacent points for corner detection
   * @return list of corner indices.
   */
  public static ArrayList<Integer> findCorners(
            java.util.List<Point2D.Double> digitizedPoints, double minAngle, double minDistance) {
        ...
        for (int i = 1, n = digitizedPoints.size(); i < n - 1; i++) {
            if (isCorner(digitizedPoints, i, minAngle, squaredDistance, previousCorner)) {
                if (intersectsPreviousCorner(digitizedPoints, i, squaredDistance, previousCorner)) {
                    cornerIndices.set(cornerIndices.size() - 1, i);
                } else {
                    cornerIndices.add(i);
                }
                previousCorner = i;
                // Calculate angle here if needed
            }
        }
    ...
    private static boolean isCorner(
            List<Point2D.Double> digitizedPoints, int i, double minAngle, double squaredDistance, int previousCorner) {
        Point2D.Double p = digitizedPoints.get(i);
        Point2D.Double prev = findPrecedingPoint(digitizedPoints, i, squaredDistance, previousCorner);
        if (prev == null) {
            return false;
        }
        Point2D.Double next = findSucceedingPoint(digitizedPoints, i, squaredDistance, n);
        if (next == null) {
            return false;
        }
        ...
        return angle < Math.PI - minAngle || angle > Math.PI + minAngle;
    }
    
    private static Point2D.Double findPrecedingPoint(
            List<Point2D.Double> digitizedPoints, int i, double squaredDistance, int previousCorner) {
        for (int j = i - 1; j >= 0; j--) {
            if (j == previousCorner
                    || v2SquaredDistanceBetween2Points(digitizedPoints.get(j), digitizedPoints.get(i)) >= squaredDistance) {
                return digitizedPoints.get(j);
            }
        }
        return null;
    }
    
    private static Point2D.Double findSucceedingPoint(
            List<Point2D.Double> digitizedPoints, int i, double squaredDistance, int n) {
        for (int j = i + 1; j < n; j++) {
            if (v2SquaredDistanceBetween2Points(digitizedPoints.get(j), digitizedPoints.get(i)) >= squaredDistance) {
                return digitizedPoints.get(j);
            }
        }
        return null;
    ...
    private static boolean intersectsPreviousCorner(
            List<Point2D.Double> digitizedPoints, int i, double squaredDistance, int previousCorner) {
        if (previousCorner < 0) {
            return false;
        }
        for (int j = i - 1; j > previousCorner; j--) {
            if (v2SquaredDistanceBetween2Points(digitizedPoints.get(j), digitizedPoints.get(i)) < squaredDistance) {
                return true;
            }
        }
...
//Refactoring end
   ...
   */
  public static ArrayList<Point2D.Double> reduceNoise(
      java.util.List<Point2D.Double> digitizedPoints, double weight) {
    java.util.ArrayList<Point2D.Double> cleaned = new ArrayList<>();
    if (digitizedPoints.size() > 0) {
      Point2D.Double prev = digitizedPoints.get(0);
      cleaned.add(prev);
   ...
   *
   * @param d Array of digitized points. Must not contain subsequent coincident points.
   ...
   * @param bezierPath
   */
  private static void addCurveTo(
      Point2D.Double[] bezCurve,
      BezierPath bezierPath,
      double errorSquared,
  ...
  private static Point2D.Double computeCenterTangent(ArrayList<Point2D.Double> d, int center) {
    Point2D.Double V1, V2, tHatCenter = new Point2D.Double();
  ...
  private static double[] chordLengthParameterize(
      ArrayList<Point2D.Double> d, int first, int last) {
    int i;
    double[] u;
    /*  Parameterization */
  ...
  private static double[] reparameterize(
      ArrayList<Point2D.Double> d, int first, int last, double[] u, Point2D.Double[] bezCurve) {
   ...
   * @param splitPoint Point of maximum error (input/output parameter, must be an array of 1)
   */
   ...
   */
  private static Point2D.Double[] generateBezier(
      ArrayList<Point2D.Double> d,
      int first,
      int last,
      double[] uPrime,
