...
public final class Geom {
   ...
   */
  public static boolean lineContainsPoint(
      int x1, int y1, int x2, int y2, int px, int py, double tolerance) {
   ...
   */
  public static boolean lineContainsPoint(
      double x1, double y1, double x2, double y2, double px, double py, double tolerance) {
   ...
   * Rectangle r1, whether or not the Rectangle r2 is on the same side of the edge as the rest of
   * this Rectangle.
   *
   * @return the logical OR of all appropriate out codes OUT_RIGHT, OUT_LEFT, OUT_BOTTOM, OUT_TOP.
   */
  ...
  /**
   * Returns a point on the edge of the shape which crosses the line from the center of the shape to
   * the specified point. If no edge crosses of the shape crosses the line, the nearest control
  ...
  public static Point2D.Double chop(Shape shape, Point2D.Double p) {
    ...
    for (; !i.isDone(); i.next()) {
      ...
      Point2D.Double chop =
          Geom.intersect(prevX, prevY, coords[0], coords[1], p.x, p.y, ctr.x, ctr.y);
      if (chop != null) {
  ...
  public static Point2D.Double cap(Point2D.Double p1, Point2D.Double p2, double radius) {
    ...
    Point2D.Double p3 = new Point2D.Double(p2.x + radius * cos(angle), p2.y + radius * sin(angle));
    return p3;
  }
  ...
  public static Point2D.Double intersect(
          ...
          if (0.0 <= r && r <= 1.0 && 0.0 <= s && s <= 1.0) {
              ...
              return calculatePointWithinLimit(xa, ya, xb, yb, xc, yc, xd, yd, r);
          }
      }

      private static Point2D.Double calculatePointWithinLimit(double xa, double ya, double xb, double yb, double xc, double yc, double xd, double yd, double r) {
          double px = xa + (xb - xa) * r;
          double py = ya + (yb - ya) * r;
...
//Refactoring end
  ...
  /**
   * compute distance of point from line segment, or Double.MAX_VALUE if perpendicular projection is
   * outside segment; or If pts on line are same, return distance from point
   */
  // from Doug Lea's PolygonFigure
   ...
   *
   * <p>This method is similar to Rectangle2D.contains, but also returns true, when rectangle1
   * contains rectangle2 and either or both of them are empty.
   *
