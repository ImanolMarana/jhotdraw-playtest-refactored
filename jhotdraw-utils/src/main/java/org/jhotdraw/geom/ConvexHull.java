...
public class ConvexHull {
  ...
  public static Point[] getConvexHull(Point[] points) {
    ...
    int lower =
        upper; // (lower - number + 1) = number of points in the lower part of the convex hull
  ...
  public static boolean isRightTurn(Point p1, Point p2, Point p3) {
    ...
    double val =
        (p2.x * p3.y + p1.x * p2.y + p3.x * p1.y) - (p2.x * p1.y + p3.x * p2.y + p1.x * p3.y);
    return val > 0;
  }
  ...
  public static Point2D.Double[] getConvexHull2D(Point2D.Double[] points) {
    ...
    return findConvexHull(sorted);
  }

  private static Point2D.Double[] findConvexHull(Point2D.Double[] sorted) {
    ...
    for (int i = sorted.length - 3; i >= 0; i--) {
      hull[lower++] = sorted[i];
      while (lower - upper > 1
          && !isRightTurn2D(hull[lower - 3], hull[lower - 2], hull[lower - 1])) {
        hull[lower - 2] = hull[lower - 1];
        lower--;
      }
    }
