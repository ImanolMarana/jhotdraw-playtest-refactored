...
public class JavaPrimitivesDOMFactory implements DOMFactory {
  ...
  public Object read(DOMInput in) throws IOException {
    ...
    if ("null".equals(tagName)) {
      return null;
    } else if (isPrimitiveType(tagName)) {
      return readPrimitive(tagName, in);
    } else if (isArrayType(tagName)) {
      return readArray(tagName, in);
    } else {
      return readComplexType(tagName, in);
    }
  }

  private boolean isPrimitiveType(String tagName) {
    return "boolean".equals(tagName)
        || "byte".equals(tagName)
        || "short".equals(tagName)
        || "int".equals(tagName)
        || "long".equals(tagName)
        || "float".equals(tagName)
        || "double".equals(tagName)
        || "string".equals(tagName)
        || "enum".equals(tagName);
  }

  private Object readPrimitive(String tagName, DOMInput in) throws IOException {
    switch (tagName) {
      case "boolean":
        return Boolean.valueOf(in.getText());
      case "byte":
        return Byte.decode(in.getText());
      case "short":
        return Short.decode(in.getText());
      case "int":
        return Integer.decode(in.getText());
      case "long":
        return Long.decode(in.getText());
      case "float":
      ...
      case "string":
        return in.getText();
      case "enum":
        return createEnum(in.getAttribute("type", (String) null), in.getText());
      default:
        throw new IllegalArgumentException("Unknown primitive type: " + tagName);
    }
  }

  private boolean isArrayType(String tagName) {
    return tagName.endsWith("Array");
  }

  private Object readArray(String tagName, DOMInput in) throws IOException {
    String elementType = tagName.substring(0, tagName.length() - "Array".length());
    int length = in.getElementCount();

    switch (elementType) {
      case "byte":
        return readByteArray(length, in);
      case "short":
        return readShortArray(length, in);
      case "int":
        return readIntArray(length, in);
      case "long":
        return readLongArray(length, in);
      case "float":
        return readFloatArray(length, in);
      case "double":
        return readDoubleArray(length, in);
      default:
        throw new IllegalArgumentException("Unsupported array type: " + tagName);
    }
  }

  private Object readComplexType(String tagName, DOMInput in) throws IOException {
    switch (tagName) {
      case "color":
        return new Color(in.getAttribute("rgba", 0xff));
      case "font":
        return new Font(
  ...
  private byte[] readByteArray(int length, DOMInput in) throws IOException {
    byte[] array = new byte[length];
    for (int i = 0; i < length; i++) {
      array[i] = ((Byte) in.readObject(i)).byteValue();
    }
    return array;
  }

  private short[] readShortArray(int length, DOMInput in) throws IOException {
    short[] array = new short[length];
    for (int i = 0; i < length; i++) {
      array[i] = ((Short) in.readObject(i)).shortValue();
    }
    return array;
  }

  private int[] readIntArray(int length, DOMInput in) throws IOException {
    int[] array = new int[length];
    for (int i = 0; i < length; i++) {
      array[i] = ((Integer) in.readObject(i)).intValue();
    }
    return array;
  }

  private long[] readLongArray(int length, DOMInput in) throws IOException {
    long[] array = new long[length];
    for (int i = 0; i < length; i++) {
      array[i] = ((Long) in.readObject(i)).longValue();
    }
    return array;
  }

  private float[] readFloatArray(int length, DOMInput in) throws IOException {
    float[] array = new float[length];
    for (int i = 0; i < length; i++) {
      array[i] = ((Float) in.readObject(i)).floatValue();
    }
    return array;
  }

  private double[] readDoubleArray(int length, DOMInput in) throws IOException {
    double[] array = new double[length];
    for (int i = 0; i < length; i++) {
      array[i] = ((Double) in.readObject(i)).doubleValue();
    }
    return array;
  }
//Refactoring end
