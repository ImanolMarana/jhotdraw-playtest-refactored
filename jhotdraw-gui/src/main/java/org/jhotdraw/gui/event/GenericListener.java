...
public abstract class GenericListener {
  ...
  private static Method getTargetMethod(
        ...
        for (Method method : methods) {
            ...
            if (!areParameterTypesMatching(method, parameterTypes)) {
                continue;
            }
            if (result != null) {
                throw new RuntimeException("ambiguous method: " + method + " vs. " + result);
            }
            result = method;
        }
    ...
    private static boolean areParameterTypesMatching(Method method, Class<?>[] parameterTypes) {
