...
public class JPopupButton extends javax.swing.JButton {
  ...
  protected void showPopup(java.awt.event.MouseEvent evt) {
    ...
    private void showPopupMenu(MouseEvent evt) {
        int x = calculatePopupX();
        int y = calculatePopupY();
        popupMenu.show(this, x, y);
        popupMenu.repaint();
    }

    private int calculatePopupX() {
        int x;
        switch (popupAnchor) {
            case SOUTH_EAST:
    ...
    private int calculatePopupY() {
        int y = getHeight();
        if (getParent() instanceof JToolBar) {
            y = adjustPopupYForToolbar();
        }
        return y;
    }

    private int adjustPopupYForToolbar() {
        ...
        if (toolbar.getOrientation() == JToolBar.VERTICAL) {
            ...
            if (toolbar.getX() > toolbar.getParent().getInsets().left) {
                y = -popupMenu.getPreferredSize().width;
            } else {
                y = getWidth();
            }
        } else {
...
//Refactoring end
  ...
  private void handleMousePressed(
      java.awt.event.MouseEvent evt) { // GEN-FIRST:event_handleMousePressed
    togglePopup(evt);
