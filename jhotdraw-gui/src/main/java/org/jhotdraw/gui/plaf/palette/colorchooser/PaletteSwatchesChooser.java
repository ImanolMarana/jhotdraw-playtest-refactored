...
public class PaletteSwatchesChooser extends AbstractColorChooserPanel {
  ...
  public void updateChooser() {
    if (updateRecursion++ == 0) {
      ...
      if (selectedColor != null) {
        int closestIndex = findClosestColorIndex(selectedColor);
        updateListSelection(closestIndex);
      }
    }
    updateRecursion--;
  }
  ...
  private int findClosestColorIndex(Color targetColor) {
    ...
    for (int i = 0, n = model.getSize(); i < n; i++) {
      ColorIcon item = (ColorIcon) model.getElementAt(i);
      Color color = item == null ? null : item.getColor();

      int squaredDistance = calculateColorDistance(targetColor, color);
  ...
  private void updateListSelection(int closestIndex) {
    if (closestIndex == -1) {
  ...
  private int calculateColorDistance(Color color1, Color color2) {
    if (color2 == null || color1.getAlpha() != color2.getAlpha()) {
