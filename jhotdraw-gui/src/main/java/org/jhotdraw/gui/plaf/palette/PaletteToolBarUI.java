...
public class PaletteToolBarUI extends ToolBarUI implements SwingConstants {
  ...
  protected void navigateFocusedComp(int direction) {
    ...
    int startIndex = focusedCompIndex;
    int currentIndex = getNextIndex(startIndex, direction);

    while (currentIndex != startIndex) {
      Component comp = toolBar.getComponentAtIndex(currentIndex);
  ...
  private int getNextIndex(int currentIndex, int direction) {
    ...
    switch (direction) {
      case EAST:
      case SOUTH:
        return (currentIndex + 1) % nComp;
      case WEST:
      case NORTH:
        return (currentIndex - 1 + nComp) % nComp;
      default:
        return currentIndex;
    }
  }
