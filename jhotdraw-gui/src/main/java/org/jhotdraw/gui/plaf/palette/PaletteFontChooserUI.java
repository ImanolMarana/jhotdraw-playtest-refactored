...
public class PaletteFontChooserUI extends FontChooserUI {
   ...
   * </ol>
   *
   * @param c the component from which this UI delegate is being removed; this argument is often
  ...
  private void doCollectionChanged() {
    ...
    FontFaceNode oldFace =
        (path != null && path.getPathCount() > 3) ? (FontFaceNode) path.getPathComponent(3) : null;
  ...
  private void setNewSelectionPath(
      FontCollectionNode newCollection, FontFamilyNode newFamily, FontFaceNode newFace) {
    FontChooserModel model = fontChooser.getModel();
    TreePath newPath;
    if (newFace != null) {
  ...
  private class FontChooserHandler implements PropertyChangeListener, TreeModelListener {
    ...
    public void propertyChange(PropertyChangeEvent evt) {
          ...
          if (isSelectionPathProperty(name)) {
              updateSelectionLists();
          } else if (isSelectedFontProperty(name)) {
              updatePreview();
          } else if (isModelProperty(name)) {
              updateModel(evt);
          }
      }
      
      private boolean isSelectionPathProperty(String name){
        return (name == null && JFontChooser.SELECTION_PATH_PROPERTY == null)
                || (name != null && name.equals(JFontChooser.SELECTION_PATH_PROPERTY));
      }
      
      private boolean isSelectedFontProperty(String name){
        return (name == null && JFontChooser.SELECTED_FONT_PROPERTY == null)
                || (name != null && name.equals(JFontChooser.SELECTED_FONT_PROPERTY));
      }
      
      private boolean isModelProperty(String name){
        return (name == null && JFontChooser.MODEL_PROPERTY == null)
                || (name != null && name.equals(JFontChooser.MODEL_PROPERTY));
      }
      ...
      private void updateModel(PropertyChangeEvent evt){
