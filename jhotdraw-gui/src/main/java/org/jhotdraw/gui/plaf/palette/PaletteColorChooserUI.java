  ...
  public class PropertyHandler implements PropertyChangeListener {
    ...
    public void propertyChange(PropertyChangeEvent e) {
        ...
        if (name.equals(JColorChooser.CHOOSER_PANELS_PROPERTY)) {
            handleChooserPanelsChange(e);
        } else if (name.equals(JColorChooser.PREVIEW_PANEL_PROPERTY)) {
            handlePreviewPanelChange(e);
        } else if ("componentOrientation".equals(name)) {
            handleComponentOrientationChange(e);
        }
    }

    private void handleChooserPanelsChange(PropertyChangeEvent e) {
        ...
        removeOldChooserPanels(oldPanels);
        addNewChooserPanels(newPanels);

        chooser.applyComponentOrientation(chooser.getComponentOrientation());
    }

    private void removeOldChooserPanels(AbstractColorChooserPanel[] oldPanels) {
        for (AbstractColorChooserPanel oldPanel : oldPanels) {
            if (oldPanel != null) {
                Container wrapper = oldPanel.getParent();
                if (wrapper != null) {
    ...
    private void addNewChooserPanels(AbstractColorChooserPanel[] newPanels) {
        mainPanel.removeAllColorChooserPanels();
        for (AbstractColorChooserPanel newPanel : newPanels) {
            if (newPanel != null) {
                mainPanel.addColorChooserPanel(newPanel);
    ...
    private void handlePreviewPanelChange(PropertyChangeEvent e) {
        if (e.getNewValue() != previewPanel) {
            installPreviewPanel();
        }
    }

    private void handleComponentOrientationChange(PropertyChangeEvent e) {
