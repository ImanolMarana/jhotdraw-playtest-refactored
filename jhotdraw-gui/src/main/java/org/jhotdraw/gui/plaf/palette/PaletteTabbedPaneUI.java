...
public class PaletteTabbedPaneUI extends BasicTabbedPaneUI {
  ...
  protected void paintContentBorder(Graphics g, int tabPlacement, int selectedIndex) {
    int x = calculateContentBorderX(tabPlacement);
    int y = calculateContentBorderY(tabPlacement);
    int w = calculateContentBorderWidth(tabPlacement);
    int h = calculateContentBorderHeight(tabPlacement);

    fillContentArea(g, selectedIndex, x, y, w, h);
  ...
  private int calculateContentBorderX(int tabPlacement) {
  ...
  private int calculateContentBorderY(int tabPlacement) {
  ...
  private int calculateContentBorderWidth(int tabPlacement) {
    int width = tabPane.getWidth();
    Insets insets = tabPane.getInsets();
    int w = width - insets.right - insets.left;
    if (tabPlacement == LEFT) {
      w -= (calculateContentBorderX(tabPlacement) - insets.left);
    } else if (tabPlacement == RIGHT) {
      w -= calculateTabAreaWidth(tabPlacement, runCount, maxTabWidth);
      if (tabsOverlapBorder) {
        w += getTabAreaInsets(tabPlacement).left;
      }
    }
    return w;
  }

  private int calculateContentBorderHeight(int tabPlacement) {
    int height = tabPane.getHeight();
    Insets insets = tabPane.getInsets();
    int h = height - insets.top - insets.bottom;
    if (tabPlacement == TOP) {
      h -= (calculateContentBorderY(tabPlacement) - insets.top);
    } else if (tabPlacement == BOTTOM) {
      h -= calculateTabAreaHeight(tabPlacement, runCount, maxTabHeight);
      if (tabsOverlapBorder) {
        h += getTabAreaInsets(tabPlacement).top;
      }
    }
    return h;
  }

  private void fillContentArea(Graphics g, int selectedIndex, int x, int y, int w, int h) {
