...
public class PaletteToolBarBorder extends AbstractBorder implements SwingConstants {
  ...
  public void paintBorder(Component component, Graphics gr, int x, int y, int w, int h) {
    ...
    if (c.getOrientation() == HORIZONTAL) {
      paintHorizontalBorder(g, c, x, y, w, h);
    } else {
      paintVerticalBorder(g, x, y, w, h);
    }
  }

  private void paintHorizontalBorder(Graphics2D g, JToolBar c, int x, int y, int w, int h) {
    if (c.getComponentOrientation().isLeftToRight()) {
      paintHorizontalLeftToRightBorder(g, c, x, y, w, h);
    } else {
      paintHorizontalRightToLeftBorder(g, c, x, y, w, h);
    }
  }

  private void paintHorizontalLeftToRightBorder(Graphics2D g, JToolBar c, int x, int y, int w, int h) {
    int barW = 18;
    int barH = h;
    int barX = 0;
    int barY = 0;
    ...
    paintHorizontalIconAndTitle(g, c, barX, barY, barW, barH);
  }

  private void paintHorizontalRightToLeftBorder(Graphics2D g, JToolBar c, int x, int y, int w, int h) {
    g.setColor(BRIGHT);
    g.fillRect(w - x - 3, y + 2, 1, h - 4);
    g.fillRect(w - x - 5, y + 2, 1, h - 4);
    g.setColor(DARK);
    g.fillRect(w - x - 2, y + 2, 1, h - 4);
    g.fillRect(w - x - 6, y + 2, 1, h - 4);
  }

  private void paintVerticalBorder(Graphics2D g, int x, int y, int w, int h) {
    g.setColor(BRIGHT);
    g.fillRect(x + 2, y + 2, w - 4, 1);
    g.fillRect(x + 2, y + 5, w - 4, 1);
    g.setColor(DARK);
    g.fillRect(x + 2, y + 3, w - 4, 1);
    g.fillRect(x + 2, y + 6, w - 4, 1);
  }

  private void paintHorizontalIconAndTitle(Graphics2D g, JToolBar c, int barX, int barY, int barW, int barH) {
    ...
    if (theTitle != null) {
      paintHorizontalTitle(g, theTitle, barX, barW, barH, icon, textIconGap);
    }
  }

  private void paintHorizontalTitle(Graphics2D g, String theTitle, int barX, int barW, int barH, Icon icon, int textIconGap) {
