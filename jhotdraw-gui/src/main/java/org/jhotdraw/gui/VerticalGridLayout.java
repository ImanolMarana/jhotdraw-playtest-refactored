...
public class VerticalGridLayout implements LayoutManager, java.io.Serializable {
   ...
   * any number of objects can be placed in a row or in a column.
   *
   * <p>All <code>VerticalGridLayout</code> constructors defer to this one.
   *
   * @param rows the rows, with the value zero meaning any number of rows
   * @param cols the columns, with the value zero meaning any number of columns
  ...
  /**
   * Sets the number of columns in this layout to the specified value. Setting the number of columns
   * has no affect on the layout if the number of rows specified by a constructor or by the
   * <tt>setRows</tt> method is non-zero. In that case, the number of columns displayed in the
   * layout is determined by the total number of components and the number of rows specified.
   *
   ...
   *
   * <p>The preferred width of a grid layout is the largest preferred width of any of the components
   * in the container times the number of columns, plus the horizontal padding times the number of
   * columns plus one, plus the left and right insets of the target container.
   *
   * <p>The preferred height of a grid layout is the largest preferred height of any of the
   * components in the container times the number of rows, plus the vertical padding times the
   * number of rows plus one, plus the top and bottom insets of the target container.
   *
   ...
   *
   * <p>The minimum width of a grid layout is the largest minimum width of any of the components in
   * the container times the number of columns, plus the horizontal padding times the number of
   * columns plus one, plus the left and right insets of the target container.
   *
   * <p>The minimum height of a grid layout is the largest minimum height of any of the components
   ...
   * the constraints of the <code>VerticalGridLayout</code> object.
   *
  ...
  public void layoutContainer(Container parent) {
        synchronized (parent.getTreeLock()) {
          ...
          if (parent.getComponentOrientation().isLeftToRight()) {
            layoutComponentsLeftToRight(parent, insets, nrows, ncols, w, h);
          } else {
            layoutComponentsRightToLeft(parent, insets, nrows, ncols, w, h);
          }
        }
      }
      
      private void layoutComponentsLeftToRight(Container parent, Insets insets, int nrows, int ncols, int w, int h) {
        for (int c = 0, x = insets.left; c < ncols; c++, x += w + hgap) {
          for (int r = 0, y = insets.top; r < nrows; r++, y += h + vgap) {
            int i = isVertical ? r + c * nrows : r * ncols + c;
      ...
      private void layoutComponentsRightToLeft(Container parent, Insets insets, int nrows, int ncols, int w, int h) {
