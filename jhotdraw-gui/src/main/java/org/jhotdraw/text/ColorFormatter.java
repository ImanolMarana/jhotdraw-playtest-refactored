...
public class ColorFormatter extends DefaultFormatter {
  ...
  protected static final Pattern RGB_INTEGER_SHORT_PATTERN = Pattern.compile(
      "^\\s*([0-9]{1,3})(?:\\s*,\\s*|\\s+)([0-9]{1,3})(?:\\s*,\\s*|\\s+)([0-9]{1,3})\\s*$");

  /** This regular expression is used for parsing the RGB_INTEGER format. */
  protected static final Pattern RGB_INTEGER_PATTERN = Pattern.compile(
      "^\\s*(?:[rR][gG][bB])?\\s*([0-9]{1,3})(?:\\s*,\\s*|\\s+)([0-9]{1,3})(?:\\s*,\\s*|\\s+)([0-9]{1,3})\\s*$");

  /** This regular expression is used for parsing the RGB_PERCENTAGE format. */
  protected static final Pattern RGB_PERCENTAGE_PATTERN = Pattern.compile(
   ...
   */
  protected static final Pattern HSB_PERCENTAGE_PATTERN = Pattern.compile(
  ...
  public ColorFormatter(Format outputFormat, boolean allowsNullValue, boolean isAdaptive) {
    ...
    try {
      lastUsedInputFormat =
          Format.valueOf(prefs.get("ColorFormatter.lastUsedInputFormat", Format.RGB_HEX.name()));
  ...
  public Object stringToValue(String str) throws ParseException {
    ...
    if (matcher.matches()) {
      ...
      try {
        return new Color(
            HSBColorSpace.getInstance(),
            new float[] {
              matcher.group(1) == null
                  ? 0f
                  : numberFormat.parse(matcher.group(1)).floatValue() / 360f,
    ...
    if (matcher.matches()) {
      ...
      try {
        return ColorUtil.toColor(ColorSpace.getInstance(ColorSpace.CS_GRAY), new float[] {
  ...
  private Format getOutputFormat(Color c) {
    Format f = outputFormat;
    if (isAdaptive) {
      f = getAdaptiveFormat(c);
    }
    return f;
  }
  ...
  private Format getAdaptiveFormat(Color c) {
    switch (c.getColorSpace().getType()) {
      ...
      default:
        return Format.RGB_INTEGER_SHORT;
    }
  }

  private String formatColor(Color c, Format f) {
    switch (f) {
      case RGB_HEX:
        return formatRGBHex(c);
      case RGB_INTEGER_SHORT:
        return formatRGBIntegerShort(c);
      case RGB_INTEGER:
        return formatRGBInteger(c);
      case RGB_PERCENTAGE:
        return formatRGBPercentage(c);
      case HSB_PERCENTAGE:
        return formatHSBPercentage(c);
      case GRAY_PERCENTAGE:
        return formatGrayPercentage(c);
      default:
        throw new IllegalArgumentException("Unsupported format: " + f);
    }
  }

  private String formatRGBHex(Color c) {
    String str = "000000" + Integer.toHexString(c.getRGB() & 0xffffff);
    return "#" + str.substring(str.length() - 6);
  }

  private String formatRGBIntegerShort(Color c) {
    return c.getRed() + " " + c.getGreen() + " " + c.getBlue();
  }

  private String formatRGBInteger(Color c) {
    return "rgb " + c.getRed() + " " + c.getGreen() + " " + c.getBlue();
  }

  private String formatRGBPercentage(Color c) {
    return "rgb% "
  ...
  private String formatHSBPercentage(Color c) {
    float[] components = getHSBComponents(c);
    return "hsb "
  ...
  private float[] getHSBComponents(Color c) {
  ...
  private String formatGrayPercentage(Color c) {
    float[] components = getGrayComponents(c);
    return "gray " + numberFormat.format(components[0] * 100) + "";
  }

  private float[] getGrayComponents(Color c) {
    if (c.getColorSpace().getType() == ColorSpace.TYPE_GRAY) {
      return c.getComponents(null);
    } else {
      return c.getColorComponents(ColorSpace.getInstance(ColorSpace.CS_GRAY), null);
    }
  }
//Refactoring end
  }
  ...
  public static AbstractFormatterFactory createFormatterFactory(
      Format outputFormat, boolean allowsNullValue, boolean isAdaptive) {
    return new DefaultFormatterFactory(
        new ColorFormatter(outputFormat, allowsNullValue, isAdaptive));
  }
}
