...
public class HSLPhysiologicColorSpace extends AbstractNamedColorSpace {
  ...
  public float[] toRGB(float[] components, float[] rgb) {
    ...
    float hk = calculateHk(hue, hueDeg);
    
    // compute p and q from saturation and lightness
    float q = calculateQ(saturation, lightness);
    float p = 2f * lightness - q;
    
    // compute red, green and blue
    float red = calculateColorComponent(hk + 1f / 3f, p, q);
    float green = calculateColorComponent(hk, p, q);
    float blue = calculateColorComponent(hk - 1f / 3f, p, q);
  ...
  private float normalizeSaturation(float saturation) {
    if (saturation > 1f) {
      saturation = 1f;
  ...
  private float normalizeLightness(float lightness) {
    if (lightness > 1f) {
      lightness = 1f;
  ...
  private float calculateHk(float hue, float hueDeg) {
  ...
  private float calculateQ(float saturation, float lightness) {
    if (lightness < 0.5f) {
  ...
  private float calculateColorComponent(float color, float p, float q) {
    if (color < 0) {
      color = color + 1f;
    }
    if (color > 1) {
