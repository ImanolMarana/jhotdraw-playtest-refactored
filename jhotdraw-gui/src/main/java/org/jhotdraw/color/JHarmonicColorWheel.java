...
public class JHarmonicColorWheel extends JColorWheel {
  ...
  private class ModelHandler implements PropertyChangeListener, ListDataListener {
    ...
    public void propertyChange(PropertyChangeEvent evt) {
      ...
      if ((name == null && HarmonicColorModel.COLOR_SPACE_PROPERTY == null)
          || (name != null && name.equals(HarmonicColorModel.COLOR_SPACE_PROPERTY))) {
        ...
        if (harmonicModel != null) {
            ...
            for (int i = harmonicModel.size() - 1; i >= 0; i--) {
                if (harmonicModel.get(i) != null) {
                    drawConnectingLine(g, center, harmonicModel.get(i));
                }
            }
            for (int i = harmonicModel.size() - 1; i >= 0; i--) {
                if (harmonicModel.get(i) != null) {
                    drawColorHandle(g, oval, i, harmonicModel.get(i));
                }
            }
        }
    }

    private void drawConnectingLine(Graphics2D g, Point center, Color color) {
        Point p = getColorLocation(color);
        g.setColor(Color.black);
        g.drawLine(center.x, center.y, p.x, p.y);
    }

    private void drawColorHandle(Graphics2D g, Ellipse2D.Float oval, int index, Color mixerColor) {
        Point p = getColorLocation(mixerColor);
        float[] comp = ColorUtil.fromColor(harmonicModel.getColorSpace(), mixerColor);
        if (index == selectedIndex) {
            drawBaseHandle(g, oval, p, baseRadius);
        }
        g.setColor(mixerColor);
        drawHandle(g, oval, p, handleRadius);
        g.setColor(Color.black);
        g.draw(oval);
        if (index == harmonicModel.getBase()) {
            drawBaseHandle(g, oval, p, baseRadius);
        }
    }

    private void drawBaseHandle(Graphics2D g, Ellipse2D.Float oval, Point p, float radius) {
        g.setColor(Color.white);
        oval.x = p.x - radius;
        oval.y = p.y - radius;
        oval.width = radius * 2f;
        oval.height = radius * 2f;
        g.fill(oval);
    }

    private void drawHandle(Graphics2D g, Ellipse2D.Float oval, Point p, float radius) {
        oval.x = p.x - radius;
        oval.y = p.y - radius;
        oval.width = radius * 2f;
        oval.height = radius * 2f;
