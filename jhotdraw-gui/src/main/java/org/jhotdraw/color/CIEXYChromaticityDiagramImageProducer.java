...
public class CIEXYChromaticityDiagramImageProducer extends MemoryImageSource {
  ...
  public void generateImage() {
        ...
        for (int iY = 0; iY <= 100; iY++) {
          ...
          for (int ix = 0; ix < w; ix++) {
            ...
            for (int iy = 0; iy < h; iy++) {
              ...
              float y = 0.9f - iy * hf;
              calculateAndSetColor(x, y, Y, ix, iy);
            }
          }
        }
      }
    
      private void calculateAndSetColor(float x, float y, float Y, int ix, int iy) {
        float z = 1f - x - y;
        float[] XYZ = calculateXYZ(x, y, z, Y);
        int alpha = calculateAlpha(XYZ);
    
        if (alpha == 255) {
          float[] rgb = calculateRGB(XYZ);
          alpha = calculateRGBAlpha(rgb);
          if (alpha == 255) {
            pixels[ix + iy * w] =
                (alpha << 24)
      ...
      private float[] calculateXYZ(float x, float y, float z, float Y) {
      ...
      private int calculateAlpha(float[] XYZ) {
        return XYZ[0] >= CEPS
                && XYZ[1] >= CEPS
                && XYZ[2] >= CEPS
                && XYZ[0] <= 1 - CEPS
      ...
      private float[] calculateRGB(float[] XYZ) {
        float[] rgb = new float[3];
        toRGB(XYZ, rgb);
        return rgb;
      }
    
      private int calculateRGBAlpha(float[] rgb) {
        return (rgb[0] >= EPS
                && rgb[1] >= EPS
                && rgb[2] >= EPS
                && rgb[0] <= 1 - EPS
