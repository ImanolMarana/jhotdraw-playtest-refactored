...
public class SVGGroupFigure extends GroupFigure implements SVGFigure {
  ...
  public Rectangle2D.Double getBounds(double scale) {
        if (cachedBounds == null) {
            cachedBounds = calculateBounds(scale);
        }
        return (Rectangle2D.Double) cachedBounds.clone();
    }
    
    private Rectangle2D.Double calculateBounds(double scale) {
        if (getChildCount() == 0) {
            return new Rectangle2D.Double();
        } else {
            Rectangle2D.Double bounds = null;
            for (Figure f : children) {
                bounds = calculateChildBounds(f, scale);
                if (cachedBounds == null) {
                    cachedBounds = bounds;
                } else {
    ...
    private Rectangle2D.Double calculateChildBounds(Figure f, double scale) {
