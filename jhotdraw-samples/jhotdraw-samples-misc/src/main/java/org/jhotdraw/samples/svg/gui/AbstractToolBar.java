...
public /*abstract*/ class AbstractToolBar extends JDisclosureToolBar implements Disposable {
  ...
  private class ProxyPanel extends JPanel {
    ...
    public void paint(Graphics g) {
        ...
        if (runner == null) {
            runner = createPanelCreationRunner(state);
            SwingUtilities.invokeLater(runner);
        }
    }
    
    private Runnable createPanelCreationRunner(final int state) {
        return new Runnable() {
            @Override
            public void run() {
                createAndReplacePanel(state);
            }
        };
    }

    private void createAndReplacePanel(int state) {
        try {
        ...
        if (parent != null) {
            replaceProxyPanel(state, parent);
        }
    }

    private void replaceProxyPanel(int state, JComponent parent) {
        ...
        if (getDisclosureState() == state) {
            addPanelToParent(state, parent, gbc);
        }
        parent.revalidate();
        ((JComponent) parent.getRootPane().getContentPane()).revalidate();
    }

    private void addPanelToParent(int state, JComponent parent, GridBagConstraints gbc) {
