...
public class ViewSourceAction extends AbstractViewAction {
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    final JDialog dialog = getOrCreateDialog(v, labels);
  ...
  private JDialog getOrCreateDialog(SVGView v, ResourceBundleUtil labels) {
    JDialog dialog;
    if (v.getClientProperty(DIALOG_CLIENT_PROPERTY) == null) {
      dialog = createDialog(v, labels);
    } else {
  ...
  private JDialog createDialog(SVGView v, ResourceBundleUtil labels) {
    final JDialog dialog = new JDialog(SwingUtilities.getWindowAncestor(v.getComponent()));
    ...
    setupDialogListeners(v, dialog, ta);

    return dialog;
  }

  private void setupDialogListeners(final SVGView v, final JDialog dialog, final JTextArea ta) {
    ...
    final PropertyChangeListener propertyChangeHandler = new PropertyChangeListener() {
      ...
      public void propertyChange(PropertyChangeEvent evt) {
        handlePropertyChange(evt, v, dialog, ta, undoableEditHandler);
      }
    };
    ...
    final Disposable disposable = new Disposable() {
      @Override
      public void dispose() {
        disposeDialogResources(v, dialog, undoableEditHandler, propertyChangeHandler);
      }
    };
  ...
  private void handlePropertyChange(PropertyChangeEvent evt, SVGView v, JDialog dialog,
      JTextArea ta, UndoableEditListener undoableEditHandler) {
  ...
  private void disposeDialogResources(SVGView v, JDialog dialog,
      UndoableEditListener undoableEditHandler, PropertyChangeListener propertyChangeHandler) {
