...
public class ODGGroupFigure extends GroupFigure implements ODGFigure {
  ...
  public Rectangle2D.Double getBounds(double scale) {
        if (cachedBounds == null) {
            cachedBounds = calculateCachedBounds(scale);
        }
        return (Rectangle2D.Double) cachedBounds.clone();
    }

    private Rectangle2D.Double calculateCachedBounds(double scale) {
        if (getChildCount() == 0) {
            return new Rectangle2D.Double();
        } else {
            Rectangle2D.Double bounds = null;
            for (Figure f : children) {
                bounds = calculateFigureBounds(f, scale);
    ...
    private Rectangle2D.Double calculateFigureBounds(Figure f, double scale) {
