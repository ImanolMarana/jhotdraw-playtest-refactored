...
public class JMixer extends javax.swing.JPanel {
  ...
  private class Preset {
    ...
    public Color getColor() {
      return new Color(
          sliderModel.getColorSpace(),
          new float[] {(index / 8) / 12f, 1f, (index % 8 + 2) / 12f},
          1f);
    }
  ...
  public JMixer() {
    ...
    setupSliderModel();
    setupListeners();
    initializeHarmonicColors();
  }

  private void setupPresets() {
    DefaultComboBoxModel<Preset> presets = new DefaultComboBoxModel<>();
  ...
  private void setupHarmonicWheel() {
    harmonicWheel.setColorSpace(HSLColorSpace.getInstance());
  }

  private void setupSliderModel() {
    sliderModel =
  ...
  private void initializeHarmonicColors() {
...
//Refactoring end
  ...
  private void ruleChangePerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_ruleChangePerformed
    updateRules();
  } // GEN-LAST:event_ruleChangePerformed

  private void mixerDisclosurePerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_mixerDisclosurePerformed
    mixerPanel.setVisible(disclosureButton.isSelected());
    mixerPanel.getParent().validate();
  } // GEN-LAST:event_mixerDisclosurePerformed

  private void systemChangePerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_systemChangePerformed
    HarmonicColorModel m = harmonicWheel.getHarmonicColorModel();
    ColorSpace sys;
