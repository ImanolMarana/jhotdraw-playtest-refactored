...
public class BezierDemo extends javax.swing.JPanel {
  ...
  private void updateExamples() {
    for (Example ex : examples) {
      ...
      if (ex.segments == null) {
        calculateSegments(ex);
      }
    }
  }
  
  private void calculateSegments(Example ex) {
  ...
  private void drawShapes(Graphics2D g, double zoomFactor) {
    ...
    if (showDigitizedCheck.isSelected()) {
      drawDigitizedPoints(g, zoomFactor);
    }
    if (showPreprocessedCheck.isSelected()) {
      drawPreprocessedPoints(g, zoomFactor);
    }
    if (showControlsCheck.isSelected()) {
      drawBezierControls(g, zoomFactor);
    }
  }
  
  private void drawPolylines(Graphics2D g) {
    g.setColor(Color.black);
    for (Example ex : examples) {
      g.draw(ex.digitized);
    }
  ...
  private void drawBezierPaths(Graphics2D g) {
  ...
  private void drawDigitizedPoints(Graphics2D g, double zoomFactor) {
    for (Example ex : examples) {
  ...
  private void drawPreprocessedPoints(Graphics2D g, double zoomFactor) {
    for (Example ex : examples) {
      g.setColor(Color.WHITE);
  ...
  private void drawBezierControls(Graphics2D g, double zoomFactor) {
    for (Example ex : examples) {
      for (BezierPath.Node node : ex.bezier.nodes()) {
        if (node.mask == BezierPath.C0_MASK) {
          ...
          drawColinearControlPoints(g, node, zoomFactor);
        } else {
          drawRegularControlPoints(g, node, zoomFactor);
        }
      }
      drawBezierNodes(g, ex.bezier, zoomFactor);
    }
  }

  private void drawColinearControlPoints(Graphics2D g, BezierPath.Node node, double zoomFactor) {
    g.setColor(Color.WHITE);
    g.fillRect(
        (int) (node.x[1] * zoomFactor - 2), (int) (node.y[1] * zoomFactor - 2), 5, 5);
    g.fillRect(
        (int) (node.x[2] * zoomFactor - 2), (int) (node.y[2] * zoomFactor - 2), 5, 5);
  ...
  private void drawRegularControlPoints(Graphics2D g, BezierPath.Node node, double zoomFactor) {
  ...
  private void drawBezierNodes(Graphics2D g, BezierPath bezier, double zoomFactor) {
...
//Refactoring end
  ...
  private void eraseButtonActionPerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_eraseButtonActionPerformed
    examples.clear();
    canvas.repaint();
  ...
  private void dumpButtonActionPerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_dumpButtonActionPerformed
    if (dumpDialog == null) {
  ...
  private void checkboxPerformed(
