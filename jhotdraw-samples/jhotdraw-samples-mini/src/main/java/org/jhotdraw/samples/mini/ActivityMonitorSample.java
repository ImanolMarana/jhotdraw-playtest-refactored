...
/**
 * This example program shows how to monitor background tasks using {@link
 * org.jhotdraw.gui.ActivityModel}, {@link org.jhotdraw.gui.JActivityIndicator} and {@link
 * org.jhotdraw.gui.JActivityWindow}.
 *
...
public class ActivityMonitorSample extends javax.swing.JFrame {
  ...
  private void startPerformed(java.awt.event.ActionEvent evt) { // GEN-FIRST:event_startPerformed
    ...
    final ActivityModel pm = createActivityModel(owner, duration, willFail, willWarn);
    
    Runnable r = new Runnable() {
      @Override
      public void run() {
        executeActivity(pm, duration, willFail, willWarn);
      }
    };
    executor.execute(r);
  } // GEN-LAST:event_startPerformed
  
  private ActivityModel createActivityModel(String owner, int duration, boolean willFail, boolean willWarn) {
    final ActivityModel pm = new DefaultActivityModel(
        owner,
        "Task #" + (++count) + ", in group " + owner + (willFail ? ", will fail" : "") + (willWarn ? ", will warn" : ""));
  ...
  private void executeActivity(ActivityModel pm, int duration, boolean willFail, boolean willWarn) {
    try {
      pm.setNote("Working...");
      if (willWarn) {
      ...
      for (long now = startTime; now < endTime && !pm.isCanceled(); now = System.currentTimeMillis()) {
        updateActivityProgress(pm, startTime, endTime, now);
        try {
          Thread.sleep(1000);
      ...
      handleActivityCompletion(pm, willFail);
    } finally {
      pm.close();
    }
  }

  private void updateActivityProgress(ActivityModel pm, long startTime, long endTime, long now) {
    pm.setValue(1 + (int) ((now - startTime) * (pm.getMaximum() - pm.getMinimum()) / (endTime - startTime)) + pm.getMinimum());
  }

  private void handleActivityCompletion(ActivityModel pm, boolean willFail) {
    if (pm.isCanceled()) {
      pm.setNote("Canceled.");
    } else {
      if (willFail) {
